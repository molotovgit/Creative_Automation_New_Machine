...
This module does NOT type the user's password. Google SSO has CAPTCHA +
2FA risks that make automation fragile and bannable. Log in manually in
the keep-alive window once; cookies persist in the profile dir.
"""
...
def login_via_google(page: Page, email: str, password: str,
    ...
    while time.time() < deadline:
        url = page.url or ""
        if CHATGPT_URL_PATTERN.search(url):
            time.sleep(2)
            if _find_one(page, SEL["prompt_input"], timeout_ms=15_000):
                return
...
def ensure_logged_in(page: Page, email: str, password: str) -> None:
    ...
    login_via_google(page, email, password)
