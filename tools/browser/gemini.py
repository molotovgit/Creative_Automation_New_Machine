...
The human_* helpers here are duplicated from flow.py for module independence;
both share the same patterns (Bezier mouse curves, jittered typing, etc.).
"""
...
# (Same patterns as tools/browser/flow.py — defeats reCAPTCHA Enterprise behavioral scoring.)
...
def login_via_google_human(page: Page, email: str, password: str, cdp_port: int,
    ...
    while time.time() < deadline:
        ...
        cur = _pick_best_page()
        cur.bring_to_front()
        if "gemini.google.com" in (cur.url or ""):
            time.sleep(3)
            print(f"[login] success — back on Gemini: {cur.url[:80]}")
            return cur
        if "accounts.google.com" in (cur.url or ""):
            _check_blockers(cur)
        time.sleep(1)
