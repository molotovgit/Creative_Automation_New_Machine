...
//  • Port 9223 → Gemini + Flow   (opens gemini + flow tabs in same window)
//
...
(async () => {
  ...
  // Port 9223 — Gemini + Flow (one window, two tabs)
  if (await checkPort(9223)) {
    console.log('  [9223] Gemini+Flow Chrome — already running. Skipping.');
  } else {
    const pid = launchChrome(chrome, 9223, profile9223, [
      'https://gemini.google.com/app',
      'https://labs.google/fx/tools/flow',
    ]);
    console.log(`  [9223] Gemini+Flow Chrome — launched (PID ${pid})`);
  ...
  console.log(`  [9223] Gemini+Flow   → ${c9223 ? 'UP ✓' : 'DOWN ✗'}`);
  ...
  console.log('');
  console.log('  Window 2 (Gemini + Flow):');
  console.log('    • Tab 1: sign in to gemini.google.com');
  console.log('    • Tab 2: sign in to labs.google/fx/tools/flow');
  console.log('    • Leave both tabs OPEN');
  console.log('');
