// Continuous VIDEO saver — scans labs.google/flow project tabs in
// .cca/flow_video_tab_map.json, downloads any rendered video to disk,
// closes the tab.
//
// Usage:
//   node scripts/save_videos_flow.cjs <prompts.json>          # exit when all done
//   node scripts/save_videos_flow.cjs <prompts.json> --watch  # poll forever
...
async function findVideoOnTab(page) {
  return page.evaluate(() => {
    for (const v of document.querySelectorAll('video')) {
      ...
      if (src && !src.startsWith('about:')) return { src, w: Math.round(r.width), h: Math.round(r.height) };
    }
    return null;
  }).catch(() => null);
}
...
(async () => {
  ...
  if (!promptsPath) {
    console.error('Usage: node save_videos_flow.cjs <prompts.json> [--watch] [--no-close]');
    process.exit(1);
  }
  ...
  console.log(`[fsave] watching ${path.basename(promptsPath)}  (${prompts.length} expected)`);
  console.log(`[fsave] output: ${outDir}`);
  console.log(`[fsave] close-tabs: ${closeTabs}, watch: ${watchMode}`);
  ...
  console.log(`[fsave] starting with ${savedIdxs.size} already saved`);
    ...
    for (const ctx of browser.browserContexts()) {
      for (const page of await ctx.pages()) {
        ...
        try {
          ...
          writeJson(SAVED_FILE, [...savedIdxs]);
          savedThisIter++;
          console.log(`[fsave] ${String(entry.idx).padStart(3, '0')} ${entry.slug}  → ${found.w}x${found.h} ${(buf.length / 1024 / 1024).toFixed(2)} MB`);
          ...
          console.log(`[fsave] ${String(entry.idx).padStart(3, '0')} download error: ${e.message}`);
        }
      }
    }

    if ((iter % 4) === 1) {
      console.log(`[fsave] iter ${iter}: ${scanned} pending, +${savedThisIter} this round  (total ${savedIdxs.size}/${prompts.length})`);
    }

    if (savedIdxs.size >= prompts.length && !watchMode) {
      console.log(`[fsave] all ${prompts.length} saved — exiting`);
      break;
    }
  ...
  console.log(`\n[fsave] DONE — ${savedIdxs.size} / ${prompts.length} saved`);
