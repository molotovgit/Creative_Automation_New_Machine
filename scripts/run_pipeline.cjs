...
//    5. ANIMATE  — turn each image into a video via Flow
//
//  PRE-REQS:
//    • ChatGPT signed in on Chrome at port 9222  (start: python chrome_keepalive.py)
//    • Gemini AND Flow signed in on Chrome at port 9223 (one window, both tabs open)
...
async function stageAnimate(promptsJson) {
  const arr = JSON.parse(fs.readFileSync(promptsJson, 'utf-8'));
  const total = arr.length;
  const subjectSlug = slugify(CONFIG.SUBJECT);
  const base = path.basename(promptsJson, '.json');
  const videosDir = path.join(REPO, 'videos', `g${CONFIG.GRADE}-${CONFIG.LANG}`, subjectSlug, base);
  const have = countFiles(videosDir, '.mp4', 50 * 1024);
  if (have >= total) {
    log('ANIMATE', `skip — ${have}/${total} videos already saved`);
    return;
  }
  log('ANIMATE', `Flow — animating ${total - have} of ${total} videos (autonomous wrapper)`);
  await runChild('node', ['scripts/run_animator.cjs', promptsJson]);
  const after = countFiles(videosDir, '.mp4', 50 * 1024);
  if (after < total) throw new Error(`animate stage finished with only ${after}/${total}`);
}
...
(async () => {
  ...
  const [chatgpt, flow] = await Promise.all([checkPort(CHATGPT_PORT), checkPort(FLOW_PORT)]);
  log('PRE', `ChatGPT Chrome :${CHATGPT_PORT} → ${chatgpt ? 'UP' : 'DOWN'}`);
  log('PRE', `Gemini+Flow Chrome :${FLOW_PORT} → ${flow    ? 'UP' : 'DOWN'}`);
  if (!chatgpt) { console.error('\n  ✗ ChatGPT Chrome not reachable. Start: python chrome_keepalive.py\n'); process.exit(2); }
  if (!flow)    { console.error('\n  ✗ Gemini/Flow Chrome not reachable. Launch real chrome.exe with --remote-debugging-port=9223 and sign in to BOTH gemini.google.com AND labs.google/fx/tools/flow\n'); process.exit(2); }
  ...
  const stages = [
    ...
    { name: '5/5 ANIMATE', fn: stageAnimate },
  ];
  ...
  console.log(`  Videos:   videos/g${CONFIG.GRADE}-${CONFIG.LANG}/${subjectSlug}/${base}/  (80 MP4s)`);
  console.log('');
})();
